...
[<img src="https://raw.githubusercontent.com/steverichey/google-play-badge-svg/266d2b2df26f10d3c00b8129a0bd9f6da6b19f00/img/en_get.svg"
    ...
    height="80">](https://play.google.com/store/apps/details?id=org.privacyguides.verifiedapps.play)
...
We publish a separate build to Google Play under the package ID `org.privacyguides.verifiedapps.play`. This is because, unfortunately, it must be signed by Google via Play App Signing, so its signing certificate hash differs from the GitHub build **by design**. The distinct package ID means the two never collide and can be installed side by side with our official GitHub release. It will also have a different launcher icon and name in your apps list in settings to distinguish them.

Using the Google Play version means trusting one additional party in the app publishing process, always consider what that means within your threat model.

The exact Android App Bundle submitted to Play is attached to each GitHub release as `VerifiedApps-<version>-play.aab` for reference. It carries `gh attestation` provenance like the APK, but it is **not** meant for sideloading (Google re-signs the APKs it distributes from this bundle).
